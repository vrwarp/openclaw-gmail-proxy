...
| Label blocklist | not in rev-14 | **Added** (`blocked_labels`): labels that DENY access and supersede both allowlists (checked first in `is_eligible`); also immutable to the agent; excluded from listings via `-label:` |
